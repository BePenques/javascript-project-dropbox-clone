class DropBoxController{
    ...
    constructor()
    {
        //atributo para se localizar na pasta 
        this.currentFolder = ['root'];
        this.onselectionchange = new Event('selectionChange');//criando um evento personalizado
        ...
        this.initEvents();
        this.readFiles();//le todos os arquivos do banco 
    }
    ...
    {     
        ...
        var firebaseConfig = {
            apiKey: "AIzaSyBn8_D2MpWPSSrcGh-EyZbmQR5LScSRjKU",
            authDomain: "dropbox-clone-1e262.firebaseapp.com",
            databaseURL: "https://dropbox-clone-1e262.firebaseio.com",
            projectId: "dropbox-clone-1e262",
            storageBucket: "dropbox-clone-1e262.appspot.com",
            messagingSenderId: "479298699711",
            appId: "1:479298699711:web:37c1a72241036dd13e610a"
        };
        // Initialize Firebase
    ...
    getSelection()//metodo que devolve qtos itens estão selecionados
    {
    ...
    initEvents()
    {
        ...
        this.btnDelete.addEventListener('click', e=>{
            ...
            this.removeTask().then(responses=>{
                ...
                responses.forEach(res=>{
                    ...
                    if(res.fields.key){//se existir remove do banco
        ...
        this.btnRename.addEventListener('click', e=>{
            ...
            //pega  a string e transforma em obj json 
            let file = JSON.parse(li.dataset.file);

            let name = prompt("Renomear o arquivo:", file.name);

            if(name){ //se name estiver preenchido

                file.name = name; //altera o nome 
                this.getFireBaseRef().child(li.dataset.key).set(file);//altera no banco de dados
            }

        });
        //toda vez que ouver alguma alteração na lista de elementos ele ouve e avisa
       ...
       //retorna as respostas de todas as promessas: 
        this.uploadTask(event.target.files).then(responses=>{

            responses.forEach(resp => {

                this.getFireBaseRef().push().set(resp.files['input-file']);//guardar no banco os dados 
    ...
    uploadComplete()//quando completa o upload 
    {
        this.modalShow(false);//esconder o modal de carregamento
        this.inputFileEl.value = '';//zera os campos 
        this.btnSendFileEl.disabled = false;//habilita botão
    }

    getFireBaseRef()//pega  areferencia do banco(onde é ora salvar )
    {
    ...
    onloadstart=function(){}) //metodo que faz o ajax 
    {
    ...
    uploadTask(files)//recebe arquivos pra fazer upload
    {
        let promises = [];

        //spreed -converte a coleção files em array
        [...files].forEach(file=>{

            let formData = new FormData();

            formData.append('input-file', file);
            //1 = nome do campo que vc quer q o post do servidor receba
            //2 = qual o arquivo
        ...
        return Promise.all(promises);//pega todas essas promesas e executa
    }
    ...
    uploadProgress(event, file)
    {
        ...
        let loaded = event.loaded;//o quanto já foi carregado em bytes
        let total = event.total;//total do arquivo em bytes 

        let porcent = parseInt((loaded / event.total) * 100);
        
        //calcular quanto tempo falta  
        let timeleft = ((100 - porcent) * timespent)/ porcent;

        //multiplica pelo tempo que já foi 
        ...
        //qto tempo ja gastou, quanto falta
        console.log(timespent, timeleft,  porcent );
    }

    formatTimeToHuman(duration)//função para converter milisegundos em hora/minutos/segundos
    {
    ...
    //metodo para retornar qual é o tipo do arquivo e qual o icone que tem que retornar

    getFileView(file,key)
    {
        let li = document.createElement('li');

        li.dataset.key = key;//para guardar o identificador do elemento 

         //no dataset so se guarda string, então é preciso fazer o stringfy           
    ...
    readFiles()//metodo para ler todos os arquivos do banco
    {

        //criar evento que monitora se existe alguma alteração na tabela, tira um snapshot sempre que tiver alteração
        ...
        li.addEventListener('click', e=>{
            ...
            if(e.shiftKey)
            {
                ...
                if(firstLi)
                {
                    ...
                    let index = [indexStart,indexEnd].sort();//ordena o array para estar sempre na ordem 
                    ...
                    return true;//para não executar o toggle
            ...
            if(!e.ctrlKey){//se a pessoa clicou no li e não tava segurando o ctrl 
                //vai remover de todos a seleção
            ...
            //seleciona somente o que foi clicado
